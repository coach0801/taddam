...
export default function PoolDetailContent({ locale, pool }: { locale: Locale; pool: Pool }) {
  ...
  const [stripeData, setStripeData] = useState<{ clientSecret: string; unitPrice: number; total: number } | null>(null)
  ...
  const handleJoin = async () => {
    ...
    setJoinError(null)
    try {
      const res = await fetch(`/api/pools/${pool.id}/commit`, {
        method: 'POST',
      ...
      setStripeData({ clientSecret: data.clientSecret, unitPrice: data.unitPrice, total: data.total })
    } catch {
  ...
  const handlePaymentSuccess = () => {
    setJoined(true)
    setTimeout(() => { window.location.href = `/${locale}/dashboard?committed=1` }, 2000)
  }
            ...
            <div className="card-elevated p-6 sticky top-24">
              {joined ? (
                ...
                <>
                  ...
                  <div className="bg-slate-50 rounded-xl p-4 mb-5 space-y-1.5 text-sm">
                    <div className="flex justify-between">
                      <span className="text-slate-500">{qty} × {formatCAD(stripeData.unitPrice)}</span>
                      <span className="font-semibold">{formatCAD(stripeData.total)}</span>
                    </div>
                    <div className="text-success-600 font-semibold text-xs text-right">
                      {locale === 'fr' ? 'Économie de' : 'You save'} {formatCAD((pool.soloPrice - stripeData.unitPrice) * qty)} vs. solo
                    </div>
                  </div>
                  <Elements
                    stripe={stripePromise}
                    options={{ clientSecret: stripeData.clientSecret, appearance: { theme: 'stripe', variables: { colorPrimary: '#4f46e5' } } }}
                  >
                    <StripePaymentForm
                ...
                <>
                  ...
                  >
                    {joining
                      ? <><Loader2 size={16} className="animate-spin" /> {locale === 'fr' ? 'Traitement…' : 'Processing…'}</>
                      : <>{t.poolDetail.joinBtn}<Shield size={16} /></>
                    }
                  </button>
